    ...
    public function callback(): void
    {
        ...
        if ($validated) {
            if ($customizable && $templateID) {
                //figure out customization tool and generate project appropriately
                if (preg_match('/^(tpl)([0-9A-Z]{3,})$/m', $templateID)) {
                $projectID = $this->new_PIE_Project($variationID, $templateID);
                $redirectUrl = "https://deveditor.peleman.com/?projectid={$projectID}";
                $productMeta = ['pie_project_id' => $projectID];
                error_log("project generated with project id {$projectID}");
                }
            }
            ...
            wp_send_json_error(array('message' => 'something fucked up'), 420);
        }
