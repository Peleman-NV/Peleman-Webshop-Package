...
abstract class PWP_Term_Handler
{
    ...
    {
        $term = $this->find_term_by_slug($slug);
        if ($term) {
    ...
    final protected function find_term_by_slug(string $slug): ?\WP_Term
    {
        $result = get_term_by('slug', $slug, $this->taxonomy, object);
        return !$result ? $result : null;
    }

    final protected function find_term_by_name(string $name): ?\WP_Term
    {
